...
| The workspace chip taken off Chat's new-session row | A mark on the shipped button beside `conversation.hero.workspace` — see [The new-session row has no project to pick](#the-new-session-row-has-no-project-to-pick) |

**Nothing in the harness is modified.** This package is a profile *bundle*: `dsh plugin` installs it into a profile and appends it to that profile's layer stack, so its one loader row is composed over the shipped tree. Removing the plugin removes the row, both segments, and every seam it used. It takes no seat in the conversation view: the workspace chip is marked rather than replaced, and the shipped preset chip stays in its own.
...
**Neither companion plugin is required, and neither appears in a top-level `inject`.** Without [omdsh-basemode](https://github.com/omdsh-plugins/omdsh-basemode) there is no switch for a segment to appear in, so the two pills are simply not there — the Chat workspace, its pin, the derived mode, and the workspace chip coming off the new-session row all go on working, because they read where the current session lives rather than the switch. Without [omdsh-shortcuts](https://github.com/omdsh-plugins/omdsh-shortcuts) the segments' tooltips name no key: this package binds nothing and registers no command, it only appends the chord that plugin reports for `mode.chat` and `mode.work` when one reaches this surface. Both services are reached from restricted fibers started inside `apply` — which is what keeps a missing companion from leaving a loader entry `pending` and failing the page's boot sweep, a dead UI rather than a missing segment.

## The new-session row has no project to pick

The harness draws a workspace chip on the blank-session screen, because a coding agent works somewhere. Chat is the posture that does not: the conversation already lives in the managed workspace, and offering to pick a different directory from that screen is offering to leave the mode. So the chip comes off the row while Chat is showing, and the harness's own **Standard mode** control is the one that remains.

The chip is not a published seat. ConversationRoot renders it as a sibling of `conversation.hero.workspace` — the menu, not the button — and replacing that slot would hide the picker without hiding the chip. So this package writes a mark on the button, the same way [omdsh-basemode](https://github.com/omdsh-plugins/omdsh-basemode) paints the sidebar: `[data-slot="conversation.hero.workspace"]` is the addressable seam, the chip is the menu-button before it, and a stylesheet this package injects takes it off the row. Uninstalling takes the stylesheet with it and the attributes stop being written.

Work still shows the chip, because Work is a conversation in a project and that is the control that names which one.

## What a chat runs

The deployment's default preset — **Standard mode**, unless the deployment ships something else — and whatever the reader picks instead, from the harness's own chip on the new-session screen. This package does not touch the composition, and does not take that chip's seat. The workspace chip beside it is a different matter — see [The new-session row has no project to pick](#the-new-session-row-has-no-project-to-pick).
...
- **The blank-chat headline and placeholder are still the shipped ones.** The workspace chip is taken off that row, but the fish headline and the composer placeholder ("Describe what you want to build") are written for work, and the harness publishes no seam for either.

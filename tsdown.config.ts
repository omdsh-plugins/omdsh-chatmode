 ...
 *   Loader from the emitted `lib/types` JavaScript.
...
const browserHalf: UserConfig = {
  ...
  // zustand/immer read process.env.NODE_ENV, and zustand's esm build probes
  // import.meta.env.MODE — which a CJS output cannot carry. Substituting both
  // here is what keeps the factory from throwing at boot.
  define: {
  ...
  plugins: [{
    ...
    // CSS Modules compiled in-bundle: importing `x.module.css` yields the
